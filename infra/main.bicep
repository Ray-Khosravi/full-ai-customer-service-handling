...
@description('Client ID of the "Nexroza Plumbing Calendar MCP" Entra app used for delegated Outlook access (public client, no secret)')
param graphClientId string = ''

@description('SMS provider used by the service-request workflow: mock | twilio | acs')
@allowed(['mock', 'twilio', 'acs'])
param smsProvider string = 'mock'

@description('Shared secret that signs inbound SMS webhooks (mock HMAC / ACS Event Grid key). Never logged.')
@secure()
param smsWebhookSecret string = ''

@description('Minutes a technician has to answer an SMS request before the next technician is tried')
param technicianReplyTimeoutMinutes int = 30

@description('Maximum technicians contacted per request before it is marked failed')
param maxTechnicianAttempts int = 3

@description('Hours a proposed time is held for customer confirmation')
param bookingHoldHours int = 12

@description('App Service Authentication behaviour for unauthenticated requests. The MCP endpoint is protected by the Functions system key instead, so anonymous must be allowed at the platform layer.')
@allowed(['AllowAnonymous', 'Return401'])
...
module api './app/api.bicep' = {
  ...
  params: {
    ...
    appSettings: union(
      !empty(graphClientId) ? { GRAPH_CLIENT_ID: graphClientId } : {},
      !empty(smsWebhookSecret) ? { SMS_WEBHOOK_SECRET: smsWebhookSecret } : {},
      {
        SMS_PROVIDER: smsProvider
        TECHNICIAN_REPLY_TIMEOUT_MINUTES: string(technicianReplyTimeoutMinutes)
        MAX_TECHNICIAN_ATTEMPTS: string(maxTechnicianAttempts)
        BOOKING_HOLD_HOURS: string(bookingHoldHours)
      }
    )
...
var storageEndpointConfig = {
  ...
  enableTable: true  // Nexroza service-request store (Azure Table Storage)
