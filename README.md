...
این پروژه یک **پذیرش هوشمند ۲۴ ساعته** برای کسب‌وکارهای خدماتی (لوله‌کشی، برق، تهویه و…) است: مشتری در چت سایت مشکلش را می‌گوید، دستیار اول **چک ایمنی** می‌کند (بوی گاز، سیل، خطر برق)، اطلاعات لازم را می‌گیرد، **تقویم زندهٔ تکنسین‌ها در Outlook** را می‌خواند، به تکنسینِ مناسب **پیامک** می‌فرستد، پاسخ او («می‌توانم ۵:۳۰») را به‌صورت **ناهمگام** دریافت می‌کند، به مشتری پیشنهاد می‌دهد و بعد از تأیید مشتری، **رزرو را در Outlook ثبت** می‌کند. مشتری هیچ‌وقت وارد حساب مایکروسافت نمی‌شود؛ صاحب کسب‌وکار فقط یک بار Outlook را وصل می‌کند.

ساخته‌شده روی Microsoft Foundry + Azure Functions (MCP) + Table Storage + Twilio/ACS، با تست‌های واحد، تست‌های end-to-end واقعی، مانیتورینگ و هزینهٔ ماهانهٔ چند دلار.
